...
import {allAsXmlRules} from "#root/app/configHandle";
...
let siteMapHandle = async (express, app, props = {}) => {
    ...
    await router.get("/product-category_sitemap.xml", async function (req, res, next) {
        try {
            const records = await getRecordsFromDataSource('ProductCategory', 'product-category');
            console.log('records', records.length)
            const collection = [];
            for (let i = 0; i < records.length; i++) {
                const url = {};
